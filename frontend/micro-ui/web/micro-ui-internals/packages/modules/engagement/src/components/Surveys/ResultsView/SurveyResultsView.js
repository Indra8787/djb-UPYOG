...
import { Card, BreakLine, CardSectionHeader, Header, MultiLink } from "@djb25/digit-ui-react-components";
...
const getUserData = async (data, tenant) => {
  ...
  userresponse = data?.map((ans) => {
    return Digit.UserService.userSearch(tenant, { uuid: [ans?.citizenId] }, {}).then((ob) => {
      return ob;
    });
  });
      ...
      return (
        ...
        <div style={{ margin: "30px" }}>
          ...
          <div className="responses-container">
            {ans?.map((el) => (
              <div className="response-result responses-container-line">
                {el}
                <BreakLine style={{ marginTop: "10px" }} />
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="date-container">
            {ans?.map((el) => (
              <div className="date-response">
                <div>{transformDate(el).date}</div>
                <div className="divide"></div>
                <div>{transformDate(el).day}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  const generateExcelObj = (ques, ans) => {
    ...
    for (let i = 0; i < countResponses; i++) {
      ...
      qStatements?.map((qs) => {
        ...
        const ansToStore =
          typeof filteredElement?.[0]?.answers?.[i] === "object"
            ? filteredElement?.[0]?.answers?.[i]?.join()?.includes("ul")
              ? "NA"
              : filteredElement?.[0]?.answers?.[i]?.join()
            : filteredElement?.[0]?.answers?.[i] === "ul"
            ? "NA"
            : filteredElement?.[0]?.answers?.[i] === ""
            ? "NA"
            : filteredElement?.[0]?.answers?.[i];
  ...
  return (
    <div className="custom-group-merge-container">
      ...
      <Card>
        ...
        {data?.map((element) => displayResult(element, element?.answers, element?.type, element?.answers?.length, t))}
      </Card>
    </div>
  );
};
